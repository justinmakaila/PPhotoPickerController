...
static const CGFloat kAnimationIntervalReset = 0.2;
static const CGFloat kAnimationIntervalTransform = 0.2;
...
@interface PPhotoCropViewController (){
    int gestureCount;
...
@implementation PPhotoCropViewController
...
- (IBAction)doneAction:(id)sender {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.view.userInteractionEnabled = YES;
            [self.delegate photoCropController:self didSelectImage:image];
        });
    });
...
- (IBAction)cancelAction:(id)sender {
    [self.delegate photoCropControllerDidCancel:self];
}
...
-(IBAction)handleRotation:(UIRotationGestureRecognizer*)recognizer{
    if([self handleGestureState:recognizer.state]) {
        ...
        CGFloat deltaX = rotationCenter.x - self.imageView.bounds.size.width / 2.0f;
        CGFloat deltaY = rotationCenter.y - self.imageView.bounds.size.height / 2.0;
